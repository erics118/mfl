...
and parse_array_suffix st source_type =
  ...
  let sz =
    ...
    | TokInt (n, _) ->
        advance st;
        n
    | _ -> error st "array size must be a constant integer"
  in
  consume st TokRBracket;
...
and parse_struct_or_var st =
  ...
  | TokLBrace -> (
      ...
      | _ -> error st "expected ';' or variable name")
  | TokSemicolon ->
...
and parse_func_tail st pos ret_type name ~is_extern =
  ...
  (* function definition *)
  | TokLBrace ->
      if is_extern then error st "extern function cannot have a body";
      consume st TokLBrace;
...
and parse_declaration ~is_extern st =
  ...
  | TokSemicolon ->
      if is_extern then
        ...
        error st "extern can only declare a function";
...
let parse (input : string) : parsed stmt =
  ...
  advance st;
  if st.cur_tok = TokEof then error st "unexpected end of input"
  else
